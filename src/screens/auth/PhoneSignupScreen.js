/**
 * Phone number entry — sends OTP via Firebase
 */
...
import { sendPhoneOTP } from '../../firebase/auth';
...
export default function PhoneSignupScreen() {
  ...
  async function handleSendOTP() {
    ...
    try {
      ...
      navigation.navigate('OTP', { phone: formattedPhone, mode: 'signup' });
    } catch (err) {
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 15, color: T.mute, marginBottom: 32 },
  phoneBox: {
  ...
  privacyText: { flex: 1, fontSize: 13, color: T.accentInk, lineHeight: 20 },
});
