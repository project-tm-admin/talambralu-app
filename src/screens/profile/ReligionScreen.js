...
export default function ReligionScreen() {
  ...
  const handleContinue = async () => {
    await save({
      religion,
      religionImportance: IMPORTANCE[importance],
      gothram,
      star,
    });
...
const styles = StyleSheet.create({
  ...
  sliderTrack: {
    height: 4, backgroundColor: T.hair2, borderRadius: 2,
  ...
  sliderThumb: {
    ...
    borderWidth: 3, borderColor: '#fff', marginLeft: -10,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.15, shadowRadius: 4, elevation: 3,
  },
  sliderLabels: { flexDirection: 'row', justifyContent: 'space-between' },
  sliderLabel: { fontSize: 11, color: T.mute, textAlign: 'center', maxWidth: 80 },
  sliderLabelActive: { color: T.accent, fontWeight: '600' },
