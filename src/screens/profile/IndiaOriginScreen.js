...
export default function IndiaOriginScreen() {
  ...
  const [tongues,   setTongues]   = useState(['Telugu']);
  ...
  const handleContinue = async () => {
    await save({
      nativeState:  homeState,
      nativeDistrict: district,
      motherTongue: tongues,
      community,
    });
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Primary label="Continue" loading={saving} onPress={handleContinue} style={{ marginTop: 8 }} />
      </ScrollView>
    </SafeAreaView>
  );
}
