  ...
  return (
    <View style={styles.section}>
      ...
      <View style={styles.chipsWrap}>
        {options.map(o => (
          <Chip key={o} label={o} selected={selected === o} onPress={() => onSelect(o)} />
        ))}
      </View>
    </View>
  );
...
export default function DietScreen() {
  ...
  const handleContinue = async () => {
    await save({ diet, drinks, smokes, exercise });
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Primary label="Continue" loading={saving} onPress={handleContinue} style={{ marginTop: 16 }} />
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 14, color: T.mute, marginBottom: 28 },
