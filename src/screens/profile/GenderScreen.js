...
export default function GenderScreen() {
  ...
  const handleContinue = async () => {
    setGender(iAm); // keep module store in sync for VisaScreen
    await save({ gender: iAm, lookingFor });
...
const styles = StyleSheet.create({
  ...
  body: { flex: 1 },
  title: {
    fontFamily: FONTS.display, fontSize: 26, color: T.ink,
    lineHeight: 32, marginBottom: 6,
  },
  subtitle: { fontSize: 13, color: T.mute, marginBottom: 16, lineHeight: 18 },
  sectionLabel: {
    fontFamily: FONTS.mono, fontSize: 10, letterSpacing: 1.2,
    color: T.mute, textTransform: 'uppercase', marginBottom: 8,
  },
});
